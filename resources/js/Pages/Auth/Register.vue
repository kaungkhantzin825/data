    ...
    <v-app theme="pipelineTheme">
        <div class="page-root">
            ...
            <div v-if="!smAndDown" class="desktop">
                ...
                <div class="d-right">
                    <div class="d-box">
                        <h1 class="d-title">Create Account</h1>
                        <p class="d-sub">Own or belong to a company? Join Pipeline.</p>

                        <form @submit.prevent="submit" class="d-form">

                            <div class="fw">
                                <div class="fbox" :class="{focused: nf, ferr: errors.name}">
                                    <v-icon icon="mdi-account-outline" size="17"
                                        :color="nf ? '#2ecc5e' : '#9ca3af'" />
                                    <input v-model="form.name" type="text"
                                        placeholder="Full name" class="fi"
                                        autocomplete="name"
                                        @focus="nf=true" @blur="nf=false" />
                                </div>
                                <span v-if="errors.name" class="emsg">{{ errors.name }}</span>
                            </div>
                            ...
                            <div class="fw">
                                <div class="fbox" :class="{focused: cf}">
                                    <v-icon icon="mdi-lock-check-outline" size="17"
                                        :color="cf ? '#2ecc5e' : '#9ca3af'" />
                                    <input v-model="form.password_confirmation"
                                        :type="showPw ? 'text' : 'password'"
                                        placeholder="Confirm password" class="fi"
                                        @focus="cf=true" @blur="cf=false" />
                                </div>
                            </div>
                        ...
                        <p class="helper">
                            ...
                            <a href="#" @click.prevent="router.visit('/login')" class="glink">Sign In</a>
                        </p>
                    </div>
                </div>
            ...
            <div v-if="smAndDown" class="mobile">
                ...
                <div class="m-card">
                    ...
                    <p class="m-sub">Own or belong to a company? Join Pipeline.</p>

                    <form @submit.prevent="submit" class="m-form">

                        <div class="fw">
                            <div class="fbox mfbox" :class="{focused: nf}">
                                <v-icon icon="mdi-account-outline" size="17"
                                    :color="nf ? '#2ecc5e' : '#9ca3af'" />
                                <input v-model="form.name" type="text"
                                    placeholder="Full name" class="fi"
                            ...
                            <span v-if="errors.name" class="emsg dark">{{ errors.name }}</span>
                        </div>
                        ...
                        <div class="fw">
                            <div class="fbox mfbox" :class="{focused: cf}">
                                <v-icon icon="mdi-lock-check-outline" size="17"
                                    :color="cf ? '#2ecc5e' : '#9ca3af'" />
                                <input v-model="form.password_confirmation"
                                    :type="showPw ? 'text' : 'password'"
...
.fbox.focused {
    border-color: #2ecc5e;
    box-shadow: 0 0 0 3px rgba(46,204,94,.14), 0 2px 12px rgba(0,0,0,.12);
}
...
.sbtn:hover:not(:disabled) {
    ...
    box-shadow: 0 8px 28px rgba(46,204,94,.48);
}
